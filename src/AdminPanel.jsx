    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 pb-24">
        <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-sm text-center">
          ...
          <p className="text-slate-500 text-sm mb-6">Área restringida para gestión de contenido.</p>
          <form onSubmit={handleLogin} className="space-y-4">
            <input 
              type="password" 
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-center font-bold tracking-widest focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="••••••••"
            />
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen pb-20">
      <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center gap-2">
        <PlusCircle className="text-blue-600" /> Cargar Nuevo Contenido
      </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        ...
        <div className={`border-2 border-dashed rounded-2xl p-6 text-center transition-colors ${formData.archivo ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200 hover:bg-slate-50'}`}>
            ...
            {formData.archivo ? (
              <>
                ...
                <div className="text-left w-full mt-2">
                  <p className="text-xs text-slate-500 font-bold uppercase">Nombre Técnico (Original):</p>
                  <p className="font-mono text-sm text-slate-700 break-all mb-2">{formData.nombreTecnico}</p>
                  
                  <div className="flex items-center gap-2 bg-white p-2 rounded-lg border border-emerald-200">
                    <CheckCircle size={16} className="text-emerald-500" />
                    <div>
                      <p className="text-[10px] text-slate-400 font-bold uppercase">Nombre Sistema (Sugerido):</p>
                      <p className="font-bold text-emerald-700">{formData.nombreSistema}</p>
                    </div>
                  </div>
                </div>
              </>
              ...
              <>
                ...
                <span className="font-bold text-blue-600">Seleccionar Archivo (Opcional)</span>
                <span className="text-xs text-slate-400">Si falla la subida, usa el campo de URL abajo</span>
              </>
            )}
          </label>
        </div>
        ...
        <div>
          <label className="block text-sm font-bold mb-1 text-slate-700 flex items-center gap-2">
            <LinkIcon size={16} className="text-slate-400" /> URL del Audio / Ruta Local
          </label>
          <input 
            type="text" 
            ...
            className="w-full p-3 border rounded-xl bg-slate-50 focus:ring-2 focus:ring-blue-500 outline-none font-medium text-sm"
          />
          <p className="text-[10px] text-slate-400 mt-1 ml-1">Úsalo si la subida automática falla o si el archivo ya está en el servidor.</p>
        </div>
        ...
        {(formData.archivo || formData.urlManual) && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div>
              <label className="block text-sm font-bold mb-1 text-slate-700 flex items-center gap-2">
                <Type size={16} className="text-blue-500" /> Título del Contenido
              </label>
              <input 
                type="text" 
                ...
                className="w-full p-3 border rounded-xl bg-slate-50 focus:ring-2 focus:ring-blue-500 outline-none font-medium"
              />
            </div>
            ...
            <div>
              <label className="block text-sm font-bold mb-1 text-slate-700 flex items-center gap-2">
                <Calendar size={16} className="text-orange-500" /> Fecha de Creación
              </label>
              <input 
                type="date" 
            ...
            <div>
              <label className="block text-sm font-bold mb-1 text-slate-700 flex items-center gap-2">
                <FileText size={16} className="text-purple-500" /> Breve Reseña (Max 20 palabras)
              </label>
              <textarea 
                ...
                className="w-full p-3 border rounded-xl bg-slate-50 h-24 text-sm leading-relaxed focus:ring-2 focus:ring-blue-500 outline-none resize-none"
              ></textarea>
            ...
            <div className="pt-2">
              <button 
                ...
                className="w-full bg-slate-900 text-white font-bold py-4 rounded-2xl shadow-lg flex items-center justify-center gap-2 hover:bg-slate-800 transition-colors disabled:opacity-50"
              >
                {isUploading ? (
                  <>
